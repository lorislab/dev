...
func CmdEnvStatus() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			apps, keys := dev.LoadApps(envConfig, flags.Tags, flags.Apps, flags.Priorities)
			for _, key := range keys {
				for _, app := range apps[key] {
					table.AddRow(app.Declaration.Priority, app.ChartRepo, app.AppName, app.Namespace, app.CurrentVersion, app.NextVersion, appActionStr[app.Action])
				}
			}
			fmt.Println(table)
		},
